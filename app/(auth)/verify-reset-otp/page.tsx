...
const VerifyResetOTP = () => {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true)
    try {
      const data = await apiClient.verifyResetOTP({ userId, otp: otpString })
  ...
  const handleResend = async () => {
    ...
    try {
      const data = await apiClient.forgotPasswordResend({ userId })
    ...
    <motion.div
      ...
      <motion.div
        ...
        <motion.div variants={fadeUp} className='mb-10 text-center'>
          <h1 className='text-5xl mb-3 font-cormorant-garamond'>Enter Code</h1>
